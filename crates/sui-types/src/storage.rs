...
pub trait RuntimeObjectResolver {
    ...
    /// Read a dynamic field object from the given parent. `child` must have an `ObjectOwner`
    /// relationship with `parent`.
    fn read_dynamic_field_object(
        &self,
        parent: &ObjectID,
        child: &ObjectID,
    ) -> SuiResult<Option<Object>> {
        self.read_child_object(Owner::ObjectOwner((*parent).into()), child)
    }
    /// Read a sent object. `child` must have an `AddressOwner` ownership type with an owner of
    /// `parent`.
    fn read_sent_object(&self, parent: &ObjectID, child: &ObjectID) -> SuiResult<Option<Object>> {
        self.read_child_object(Owner::AddressOwner((*parent).into()), child)
    }
}
