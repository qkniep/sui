...
impl GasCharger {
    ...
    pub fn move_gas_status(&mut self) -> &mut GasStatus {
    ...
    pub fn charge_gas<T>(
        ...
        if let Some(gas_object_id) = self.smashed_gas_coin {
            if Self::dont_charge_budget_on_storage_oog(self.gas_model_version) {
    ...
    pub(crate) fn dont_charge_budget_on_storage_oog(gas_model_version: u64) -> bool {
        gas_model_version >= 4
    }
}
...
impl SuiGasStatus {
    pub fn new(
        ...
        if Self::gas_price_too_high(config.gas_model_version())
    ...
    fn gas_price_too_high(gas_model_version: u64) -> bool {
        gas_model_version >= 4
    }
}
