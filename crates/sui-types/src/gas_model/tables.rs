...
impl GasStatus {
    ...
    pub fn charge_bytes(&mut self, size: usize, cost_per_byte: u64) -> PartialVMResult<()> {
        let computation_cost = if Self::charge_input_as_memory(self.gas_model_version) {
    ...
    // If returns true, input object bytes are treated as memory allocated in Move and
    // charged according to the bucket they end up in.
    fn charge_input_as_memory(gas_model_version: u64) -> bool {
        gas_model_version == 4
    }
}
